<template>
    <AppLayout>
        <PageTitle title="Edit Insurance">
            <template #actions>
                <Button
...
import { updateInsuranceById } from '@admin/services/insuranceService'
...
const { initialValues, formKey, insuranceId, getInsurance } = useInsurance()

const onFormSubmit = async ({ valid, values, errors }) => {
    if (valid) {
        try {
            await updateInsuranceById(insuranceId, values)

            customToast.success('Insurance updated successfully!')
