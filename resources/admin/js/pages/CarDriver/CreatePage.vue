<template>
    <AppLayout>
        <PageTitle title="Add new Driver">
            <template #back>
                <Button
...
import { createCarDriver } from '@admin/services/carDriverService'
import { carDriverValidator } from '@admin/validators/carDriverValidator'

const { toCarDriversList } = useRedirects()
const { customToast } = useCustomToast()
const { initialValues, maxBirthDate, maxLicenceDate } = useCarDriver()
...
const onFormSubmit = async ({ valid, values }) => {
    if (valid) {
        try {
            ...
            await createCarDriver(values)

            customToast.success('Car driver created successfully!')
